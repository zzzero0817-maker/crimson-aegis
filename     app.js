...
function renderFeatured() {
  ...
  const focusCard = document.querySelector('.acsl-focus');
  if (focusCard) {
    const eyebrow = focusCard.querySelector('.section-title-row .eyebrow');
    const title = focusCard.querySelector('.section-title-row h2');
    const metricValues = focusCard.querySelectorAll('.metric-grid strong');
    const rangeLabels = focusCard.querySelectorAll('.range-labels span');
    const detailButton = focusCard.querySelector('[data-page="acsl"]');
    const warning = buy > 0 ? buy * 0.9 : price * 0.9;
    const target = buy > 0 ? buy * 1.2 : price * 1.2;

    if (eyebrow) eyebrow.textContent = p.code;
    if (title) title.textContent = `${p.name}重点監視`;
    if (metricValues[1]) metricValues[1].textContent = usd(buy);
    if (rangeLabels[0]) rangeLabels[0].textContent = `警戒 ${usd(warning)}`;
    if (rangeLabels[1]) rangeLabels[1].textContent = `取得 ${usd(buy)}`;
    if (rangeLabels[2]) rangeLabels[2].textContent = `目標 ${usd(target)}`;
    if (detailButton) detailButton.textContent = `${p.code}詳細を見る`;
  }

  const detailPage = document.getElementById('acsl');
  if (detailPage) {
    const pageEyebrow = detailPage.querySelector('.page-title .eyebrow');
    const pageTitle = detailPage.querySelector('.page-title h2');
    const holdingTitle = detailPage.querySelector('.section-title-row h3');
    const holdingMeta = detailPage.querySelector('.section-title-row .muted');
    if (pageEyebrow) pageEyebrow.textContent = `${p.code} COMMAND CENTER`;
    if (pageTitle) pageTitle.textContent = `${p.name}専用分析`;
    if (holdingTitle) holdingTitle.textContent = `${p.code} ${p.name}`;
    if (holdingMeta) holdingMeta.textContent = `取得価格 ${usd(buy)} / ${shares}株`;
  }
